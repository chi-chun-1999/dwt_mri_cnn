...
from src.model.cnn_model import EasyConvNet
...
# %% 
        ...
        dwt_cnn = models.resnet18(pretrained=False)
        num_ftrs = dwt_cnn.fc.in_features
        dwt_cnn.fc = nn.Linear(num_ftrs, 2)
        dwt_cnn = dwt_cnn.to(device)
        ...
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        #dwt_cnn = EasyConvNet(first_dense_layer_par=54).to(device)
        #dwt_cnn = DwtConvNet2Class().to(device)
        #summary(dwt_cnn,(3,104,88))
        criterion = nn.CrossEntropyLoss()
